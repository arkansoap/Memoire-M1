...
```{r}
levels(datRecid$train$is_violent_recid)
datrecidTEST <- datRecid$train
datrecidTEST$is_violent_recid <-  recode_factor(datrecidTEST$is_violent_recid, "0" = "nonrecid", "1" = "recid")
```

Autres tests : 

```{r}
TestCost <- train(is_violent_recid~.,
                  data = datrecidTEST,
                  method = "svmRadial",
                  class.weights = c(nonrecid = 1, recid = 5)
                  )
```

```{r}
predAAAA <- predict(TestCost, datrecidTEST)
```

```{r}
perf.measure(predAAAA, datrecidTEST, "is_violent_recid")
```
