...
```{r}
Modelglm2 <- glmnet(popularity~., data = datas$train , family = "binomial", alpha = 0.5)
```

```{r}
predict(Modelglm2, datas$test, type = "class", s=1)
```

```{r}
glmnet(popularity~., data = datas$train , family = "multinomial")
```

```{r}
table(predict(Modelglm2, datas$test, type = "class", s=1))
```
